...
class Tags {
  ...
  removeItem(value, noEvents = false) {
    ...
    // CUSTOM CODE ADDED
    // custom event to hook into so that this data can be seen when removed
    if(!noEvents) {
      const event = CreateEventTagRemoved(value, this._selectElement.getAttribute("data-tags-for"));
      this._selectElement.dispatchEvent(event);
    }
  }
}
